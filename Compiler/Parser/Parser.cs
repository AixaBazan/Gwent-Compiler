class Parser
{
    ...
    // if (match(PRINT))
    // returnprintStatement(); return expressionStatement();
    ...
    //     //El lo hace como una expresion
    //     if(Stream.Match(TokenType.Identifier))
    //     {
    //         exp2 = new Variable(Stream.Previous(), Stream.Previous().Location);
    //     }
    //     else throw new CompilingError(Stream.Previous().Location, ErrorCode.Expected, "Se espera una variable despues de In");
    //     Stmt body = null;
    ...
    //     return new For(exp1, exp2, body, loc);
    // }
    private Stmt While()
    {
        CodeLocation loc = Stream.Previous().Location;
    ...
    public Stmt VarDeclaration() 
    {
        ...
        return new Var(Id, op, initializer, Id.Location);
    }
    // private List<Stmt> Block() 
    // {
    //     List<Stmt> statements = new List<Stmt>();
    //     while(!Stream.Check(TokenValue.OpenCurlyBracket) && !Stream.End) 
    //     {
    //         statements.Add(Declaration());
    //     }
    //     Stream.Consume(TokenValue.ClosedCurlyBracket, "Se espera '}' después del bloque."); 
    //     return statements;
    // }
    ...
    //     try
    //     {
    //         if (Stream.CheckType(TokenType.Identifier)) return VarDeclaration();
    //         return null;
    //     }
    //     catch (CompilingError error)
    //     {
    ...
    private Expression Primary()
    {
        ...
        if (Stream.Match(TokenType.Identifier)) 
        {
            ...
            return new Variable(variable, Stream.Previous().Location);
        }
